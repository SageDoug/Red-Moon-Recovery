...
def build_prediction_prompt(user_id):
    conn = get_db()
    ...
    prompt = (
        ...
        "Analyze the following data and provide detailed, evidence-based predictions and recommendations.\n\n"
        "Today's date: " + today + "\n\n"
        "=== ATHLETE PROFILE ===\n" + profile_text + "\n"
        "=== JOURNAL DATA (last 14 entries, newest first) ===\n" + journal_text + "\n"
        "=== YOUR TASK ===\n"
        "Provide a comprehensive prediction and recommendation report in the following JSON format exactly:\n\n"
        "{\n"
        '  "next_period_predicted": "YYYY-MM-DD",\n'
        '  "next_period_confidence": "high|medium|low",\n'
        '  "period_shift_factors": ["factor1", "factor2"],\n'
        '  "ovulation_estimated": "YYYY-MM-DD",\n'
        '  "current_phase": "menstrual|follicular|ovulatory|luteal|unknown",\n'
        '  "current_phase_day": 1,\n'
        '  "event_phase_prediction": "phase name or null",\n'
        '  "event_cycle_day_prediction": 1,\n'
        '  "event_recommendations": "specific advice for their event",\n'
        '  "training_this_week": "specific training recommendation",\n'
        '  "injury_flags": ["flag1", "flag2"],\n'
        '  "environmental_impact": "assessment of environmental factors on cycle",\n'
        '  "pattern_alerts": ["alert1", "alert2"],\n'
        '  "nutrition_focus": "current nutrition recommendation",\n'
        '  "calendar_suggestions": [\n'
        '    {"title": "event title", "date": "YYYY-MM-DD", "type": "rest|training|nutrition|warning|prediction", "description": "brief description"},\n'
        '    {"title": "event title", "date": "YYYY-MM-DD", "type": "rest|training|nutrition|warning|prediction", "description": "brief description"}\n'
        "  ],\n"
        '  "summary": "2-3 sentence overall summary for the athlete"\n'
        "}\n\n"
        "Base your predictions on:\n"
        "1. The logged cycle data and phase patterns\n"
        "2. Environmental factors (heat, humidity, travel stress) and their known effects on cycle timing\n"
        "3. Training load, stress levels, and sleep quality (all can delay ovulation)\n"
        "4. Any injuries noted and how they should affect training recommendations\n"
        "5. Flow characteristics and how they compare to typical patterns\n"
        "6. Nutritional factors that might affect cycle regularity\n\n"
        "Return ONLY the JSON object. No explanation, no markdown, no backticks."
    )
    return prompt


def run_claude_predictor(user_id):
    api_key = os.environ.get('ANTHROPIC_API_KEY')
    if not api_key:
        return None, "No ANTHROPIC_API_KEY set — using built-in predictions."

    prompt = build_prediction_prompt(user_id)
    ...
    try:
        ...
        conn.execute(
            ...
            (user_id, 'full_cycle', json.dumps(result),
             result.get('next_period_predicted'), result.get('next_period_confidence'))
        )
        conn.commit()

        # Save calendar suggestions from Claude
        if result.get('calendar_suggestions'):
            for suggestion in result['calendar_suggestions']:
                try:
                    conn.execute('''INSERT INTO calendar_events
                        ...
                        VALUES (?,?,?,?,?,?,?,?,?)''',
                        (user_id, user_id, 'ai',
                         suggestion.get('title', 'AI Suggestion'),
                         suggestion.get('description', ''),
                         suggestion.get('date', ''),
                         suggestion.get('type', 'prediction'),
                         1, '#8B5CF6'))
                except Exception:
                    pass
        conn.commit()
        ...
        return None, "Could not parse prediction response: " + str(e)
...
def run_builtin_predictor(user_id):
    conn = get_db()
    profile = conn.execute('SELECT * FROM profiles WHERE user_id = ?', (user_id,)).fetchone()
    entries = conn.execute(
    ...
    try:
        last = datetime.strptime(profile['last_period_start'], '%Y-%m-%d')
        cycle_len = int(profile.get('avg_cycle_length') or 28)
        today = datetime.now()

        # Estimate adjustments from logged factors
        delay = 0
        shift_factors = []
        alerts = []
        injury_flags = []
        ...
        if recent:
            ...
            if stress_vals and sum(stress_vals) / len(stress_vals) >= 7:
                delay += 2
                shift_factors.append("High stress levels (avg " + str(round(sum(stress_vals)/len(stress_vals), 1)) + "/10) — possible +1-3 day delay")
            ...
            if sleep_vals and sum(sleep_vals) / len(sleep_vals) < 5:
                ...
                alerts.append("Chronically low sleep quality is affecting recovery and may affect cycle timing")

            for e in recent:
                if e.get('environmental_notes') and str(e['environmental_notes']).strip():
                    shift_factors.append("Environmental factors logged: " + str(e['environmental_notes']))
                    break

            for e in recent:
                if e.get('injuries') and str(e['injuries']).strip() not in ['', 'none', 'None']:
        ...
        if profile.get('event_date'):
            try:
                event = datetime.strptime(profile['event_date'], '%Y-%m-%d')
                ...
                if event_day <= 5:
                    ...
                    event_rec = 'Rising energy — good performance expected. Train hard in the week before to arrive sharp.'
                elif event_day <= 17:
        ...
        ovulation_date = ovulation_est.strftime('%Y-%m-%d')
        ...
        result = {
            ...
            'ovulation_estimated': ovulation_date,
            ...
            'training_this_week': 'Focus on ' + current_phase + '-phase appropriate training.',
            'injury_flags': injury_flags,
            'environmental_impact': 'Based on logged data.' if shift_factors else 'No significant environmental factors logged.',
            ...
            try:
                if suggestion.get('date'):
                    conn.execute('''INSERT INTO calendar_events
                        ...
                        VALUES (?,?,?,?,?,?,?,?,?)''',
                        (user_id, user_id, 'ai',
                         suggestion['title'], suggestion.get('description', ''),
                         suggestion['date'], suggestion.get('type', 'prediction'),
                         1, '#8B5CF6'))
            except Exception:
                pass
        conn.commit()
...
def get_prediction(user_id):
    result, error = run_claude_predictor(user_id)
    if result:
        return result, None, 'claude'
    result, error2 = run_builtin_predictor(user_id)
