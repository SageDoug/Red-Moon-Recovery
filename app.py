...
app.secret_key = os.environ.get('SECRET_KEY', 'redmoon-secret-key-2024')

# Initialize database on startup
init_db()
print("Red Moon Recovery - Database ready")
...
def create_team():
    ...
    conn = get_db()
    cur = conn.execute(
        'INSERT INTO teams (name, coach_id, invite_code) VALUES (?,?,?)',
...
def join_team():
    d = request.json or {}
    invite_code = str(d.get('invite_code') or '').strip().upper()
    if not invite_code:
...
def team_members(team_id):
    ...
    if not member:
        ...
        return jsonify({'error': 'Access denied'}), 403
    members = conn.execute(
        'SELECT u.id, u.username, u.role, tm.joined_at FROM users u JOIN team_members tm ON tm.user_id = u.id WHERE tm.team_id = ?',
        (team_id,)
    ).fetchall()
    conn.close()
...
def athlete_data(team_id, athlete_id):
    conn = get_db()
    me_member = conn.execute(
        'SELECT * FROM team_members WHERE team_id = ? AND user_id = ?',
        (team_id, session['user_id'])
    ).fetchone()
    athlete_member = conn.execute(
        'SELECT * FROM team_members WHERE team_id = ? AND user_id = ?',
        (team_id, athlete_id)
    ).fetchone()
...
def save_profile():
    ...
    if existing:
        conn.execute(
            'UPDATE profiles SET name=?,age=?,cycle_status=?,goals=?,sport=?,event_date=?,desired_phase=?,cycles_data=?,training_load=?,acl_history=?,avg_cycle_length=?,last_period_start=?,updated_at=datetime("now") WHERE user_id=?',
        ...
        conn.execute(
            'INSERT INTO profiles (user_id,name,age,cycle_status,goals,sport,event_date,desired_phase,cycles_data,training_load,acl_history,avg_cycle_length,last_period_start) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)',
...
def save_journal():
    ...
    conn = get_db()
    conn.execute(
        'INSERT INTO journal_entries (user_id,entry_date,cycle_day,phase,hrv,sleep_quality,sleep_hours,energy,soreness,pain_notes,workout,rpe,motivation,perf_notes,flow,cramps,mucus,digestion,symptom_time,mood,cognitive,social,cravings,hydration,recovery_steps,environmental_temp,environmental_humidity,environmental_notes,injuries,stress_level) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
...
def chat_send():
    ...
    try:
        ...
        reply = "I had trouble processing that. Please try again. (Error: " + str(e) + ")"
...
def predict():
    ...
    try:
        ...
        return jsonify({'success': False, 'error': 'Prediction error: ' + str(e)})
...
def get_predictions():
    conn = get_db()
    rows = conn.execute(
        'SELECT * FROM ai_predictions WHERE user_id = ? ORDER BY created_at DESC LIMIT 5',
        (session['user_id'],)
    ).fetchall()
    conn.close()
    results = []
...
if __name__ == '__main__':
    ...
    print("Starting Red Moon Recovery on port " + str(port))
