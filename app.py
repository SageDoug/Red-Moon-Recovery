...
# Initialise DB — CURRENT_TIMESTAMP fix means this now works reliably
init_db()
...
def save_profile():
    ...
    if existing:
        conn.execute(
            'UPDATE profiles SET name=?,age=?,cycle_status=?,goals=?,sport=?,event_date=?,desired_phase=?,cycles_data=?,training_load=?,acl_history=?,avg_cycle_length=?,last_period_start=?,updated_at=CURRENT_TIMESTAMP WHERE user_id=?',
...
def chat_send():
    ...
    if sid:
        ...
        sess = conn.execute('SELECT * FROM chat_sessions WHERE user_id=? ORDER BY created_at DESC LIMIT 1',
                            (uid,)).fetchone()
    if not sess:
        cur = conn.execute('INSERT INTO chat_sessions (user_id, session_name) VALUES (?,?)',
                           (uid, 'Luna Chat'))
...
def analyze_patterns():
    uid = session['user_id']
    cpp_result, err = run_cpp_analyzer(uid)
    if cpp_result:
        return jsonify({'success': True, 'analysis': cpp_result})
    return jsonify({'success': False, 'error': err})


@app.route('/api/analyze/cycle', methods=['GET'])
@require_auth
def analyze_cycle():
    uid = session['user_id']
    java_result, err = run_java_engine(uid)
    if java_result:
        return jsonify({'success': True, 'prediction': java_result})
    return jsonify({'success': False, 'error': err})
